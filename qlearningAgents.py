...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        for i in self.getLegalActions(state):
            maxQ = max(maxQ , self.getQValue(state,i))
    ...
    def computeActionFromQValues(self, state):
        ...
        bestActions = []
        for i in self.getLegalActions(state):
            if bestQ == self.getQValue(state,i):
    ...
    def update(self, state, action, nextState, reward):
        ...
        new = reward + (self.discount * self.computeValueFromQValues(nextState))
        #util.raiseNotDefined()
        self.values[state,action] = (1-self.alpha)*old + self.alpha*new
